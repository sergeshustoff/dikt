...
class DefaultArgumentTest {
    ...
    fun `allow default arguments in injected constructors`() {
        val result = compile(
            ...
            class MyModule {
                @Create fun injectable(dependency: String): TestObject
            }
            """
            )
        )
    ...
    fun `allow default arguments in generated methods`() {
        val result = compile(
            ...
            class MyModule {
                @Create fun injectable(dependency: String = "default"): TestObject
            }
            """
            )
        )
    ...
    fun `use default value if not provided from dependencies`() {
        val result = compile(
            ...
            class MyModule {
                @Create fun injectable(index: Int): TestObject
            }
            """
            )
        )
